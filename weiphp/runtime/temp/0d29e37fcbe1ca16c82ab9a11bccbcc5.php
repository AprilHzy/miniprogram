...
<div class="navbar navbar-inverse navbar-fixed-top">
  ...
  <div class="setting_step app_setting">
         <a class="step step_cur_1" href="<?php echo U('weixin/Publics/step_miniapp_0',['id' => $id ]); ?>">1.增加基本信息</a>
         <a class="step " style="cursor:deault" >2.服务器域名</a>
  </div>     
  
  <div class="tab-content"> 
    <!-- 表单 -->
    <form class="form-horizontal bind_step_miniapp_form" method="post" action="<?php echo U('weixin/Publics/step_miniapp_0'); ?>" id="form" style="overflow:hidden; zoom:1">
      <!-- 基础文档模型 -->
      <div class="tab-pane in tab1" id="tab1">
        <div class="item_wrap">
      	 <div class="form-item cf">
          <label class="item-label"> <span class="need_flag">*</span> 小程序类型 <span class="check-tips"> （请正确选择，小程序类型对应的接口如果没有权限，相关的功能将不显示）</span></label>
          <div class="controls">
                <select name="type">
                   <option value="1" <?php if($info['type'] == '1'): ?>selected<?php endif; ?> >认证小程序</option>
                   <option value="0" <?php if($info['type'] == '0'): ?>selected<?php endif; ?> >未认证小程序</option>
                </select>
          </div>
        </div>        
      	 <div class="form-item cf toggle-public_name">
          <label class="item-label"> <span class="need_flag">*</span> 小程序名称 </label>
          <div class="controls">
            <input type="text" value="<?php echo htmlentities((isset($info['public_name']) && ($info['public_name'] !== '')?$info['public_name']:'')); ?>" name="public_name" class="text input-large">
          </div>
        </div>
        <div class="form-item cf toggle-public_id">
          ...
          <div class="controls">
            <input type="text" value="<?php echo htmlentities((isset($info['public_id']) && ($info['public_id'] !== '')?$info['public_id']:'')); ?>" name="public_id" class="text input-large">
          </div>
        </div>  
        <div class="form-item cf toggle-appid">
          <label class="item-label"> AppID <span class="check-tips"> （小程序ID） </span></label>
          <div class="controls">
            <input type="text" value="<?php echo htmlentities((isset($info['appid']) && ($info['appid'] !== '')?$info['appid']:'')); ?>" name="appid" class="text input-large">
          </div>
        </div>
        <div class="form-item cf toggle-secret">
          <label class="item-label"> AppSecret <span class="check-tips"> （小程序密钥） </span></label>
          <div class="controls">
            <input type="text" value="<?php echo htmlentities((isset($info['secret']) && ($info['secret'] !== '')?$info['secret']:'')); ?>" name="secret" class="text input-large">
          </div>
        </div>                        
        </div>     
        <div class="form-item cf mt_10 bind_step_miniapp_form_next_item">
          <input type="hidden" name="id" value="<?php echo htmlentities((isset($id) && ($id !== '')?$id:0)); ?>">
