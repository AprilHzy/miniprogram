...
<?php endif; ?>
      <div class="table-bar">
        <div class="fl">
          ...
          <?php if($syc_wechat): ?><a href="<?php echo U('syc_auth_group'); ?>" class="btn tongbu">一键同步微信公众号粉丝</a> &nbsp;
             <?php endif; if(in_array('in_blacklist',$db_fields)): ?>
        ...
        <div class="search-form fr cf">
          ...
          <div class="sleft">
            <input type="text" placeholder="请输入昵称搜索" value="<?php echo htmlentities($search_key); ?>" class="search-input" name="key">
      ...
      <div class="row justify-content-between" style="margin-left: 0">
                <div class="form-inline form-group ml-4">
                          ...
                          <input style="width:175px" type="datetime" autocomplete="off" name="s_time" class="text time mr-1 ml-1" value="<?php echo input('s_time'); ?>" placeholder="请选择时间" />
                          ：
                          <input style="width:175px"  type="datetime" autocomplete="off" name="e_time" class="text time mr-1 ml-1" value="<?php echo input('e_time'); ?>" placeholder="请选择时间" />
                    <pre>  </pre>
            ...
            <tbody>
              <?php if(is_array($list_data) || $list_data instanceof \think\Collection || $list_data instanceof \think\Paginator): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                <tr>
                  ...
                  <td><?php echo htmlentities($vo['country']); ?><?php echo htmlentities($vo['province']); ?><?php echo htmlentities($vo['city']); ?></td>
                  ...
                  <?php if(!(empty($vo['in_blacklist']) || (($vo['in_blacklist'] instanceof \think\Collection || $vo['in_blacklist'] instanceof \think\Paginator ) && $vo['in_blacklist']->isEmpty()))): ?>
                  	<a onclick="if(confirm('确定移出黑名单么？'))window.location.href = '<?php echo U ( 'weixin/UserCenter/outbacklist',array('bopenid'=>$vo['openid'],'uid'=>$vo['uid'])); ?>'" target="_self">移出黑名单</a>
                  <?php endif; ?>
                  <!--<a href="<?php echo U ( 'weixin/UserCenter/edit',array('uid'=>$vo['uid'])); ?>" target="_self">编辑</a>-->
  ...
  $("#search").click(function(){
	...
	$('#batchblacklist').click(function(){
		...
		if(confirm('确定将用户加入黑名单么？')){
			that = this;
			$(that).addClass('disabled').attr('autocomplete','off').prop('disabled',true);
			target = '<?php echo U("addblacklist"); ?>';
			 $.post(target,query).success(function(data){
				location.reload();
				$('.thinkbox-modal-blackout-default,.thinkbox-default').hide();
		     });
