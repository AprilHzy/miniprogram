...
class WeixinMessage extends Home
{
    ...
    public function lists()
    {
        ...
        if (isset($list_data['list_data']) && is_array($list_data['list_data'])) {
            foreach ($list_data['list_data'] as $k => &$v) {
                ...
                $v['Content'] = $content = $this->_deal_content($v);
    ...
    {
        $dd = json_decode($data['Content'], true);
        !is_array($dd) && $dd = [];
            ...
            case 'voice':
                ...
                if (empty($data['Content'])) {
                    ...
                    if ($fileid) {
                        ...
                        $addContent['title'] = $file_voice['name'];
                        $dd = array_merge($dd, $addContent);
                        $save['Content'] = json_url($addContent);
                        M('weixin_message')->where('id', $data['id'])->update($save);
                    }
                } else {
                    $con = json_decode($data['Content'], true);
                    $dd['id'] = $con['file_id'];
            ...
            case 'video':
                ...
                if (empty($data['Content'])) {
                    ...
                    if ($fileid) {
                        ...
                        $addContent['file_id'] = $dd['id'];
                        $addContent['title'] = $file_video['name'];
            ...
            default:
                ...
                if (!empty($content)) {
                    if (isset($content['image'])) {
                        ...
                        // 保存到Content里
                        $save['MsgType'] = $addContent['msgtype'] = 'image';
                        $addContent['image']['media_id'] = $imagemap['media_id'];
                        $addContent['picurl'] = $dd['url'];
                        $dd = array_merge($dd, $addContent);
                        $save['Content'] = json_url($addContent);
                        $save['MediaId'] = $imagemap['media_id'];
                        M('weixin_message')->where('id', $data['id'])->update($save);
                    } else
    ...
    function reply()
    {
        ...
        switch ($msg_type) {
            ...
            case 'image':
                ...
                if ($image_cover_id) {
                    $mediaId = D('common/Custom')->get_image_media_id($image_cover_id, 'image', false);
                    ...
                    if ($imageMaterial['media_id']) {
                        ...
                        $mediaId = D('common/Custom')->get_image_media_id($imageMaterial['cover_id']);
                        M('material_image')->where('id', $imageMaterial['id'])->update(['media_id' => $mediaId]);
                    }
                }
                if (empty($mediaId)) {
                    $this->error('该图片medial_id获取失败');
                }
                // 新增图片素材
                $param['msgtype'] = 'image';
        ...
        $follow = D('common/Follow')->where('openid', $param['touser'])->find();

        $url = 'https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token=' . get_access_token($follow['pbid']);
        ...
        $data['ToUserName'] = get_pbid_appinfo($follow['pbid'], 'public_id');
